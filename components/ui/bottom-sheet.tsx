...
const bottomSheetBackdropStyle = 'absolute inset-0 flex-1 touch-none select-none bg-black opacity-0'
...
const bottomSheetItemStyle = 'p-3 flex-row items-center rounded-sm w-full disabled:opacity-0.4 web:pointer-events-auto disabled:cursor-not-allowed hover:bg-background-50 active:bg-background-100 focus:bg-background-100 web:focus-visible:bg-background-100'
...
export const BottomSheetTrigger = ({
    ...
    return (
        <Pressable
            ...
            {...props}
            className={cn(bottomSheetTriggerStyle, className)}            
        >
            {props.children}
        </Pressable>
    );
...
export const BottomSheetBackdrop = ({
    ...
    return (
        <GorhomBottomSheetBackdrop
            // @ts-ignore
            className={cn(bottomSheetBackdropStyle, className)}
...
export const BottomSheetDragIndicator = ({
    ...
    return (
        <BottomSheetHandle
            {...props}
            // @ts-ignore
            className={cn(bottomSheetIndicatorStyle, className)}            
        >
            {children}
        </BottomSheetHandle>
    );
    ...
    if (Platform.OS === 'web')
        return (
            <GorhomBottomSheetView
                ...
                // @ts-ignore
                {...keyDownHandlers}
                className={cn(bottomSheetContentStyle, props.className)}
            >
                {visible && (
...
export const BottomSheetItem = ({
    ...
    return (
        <Pressable
            {...props}
            className={cn(bottomSheetItemStyle, className)}
            onPress={(e) => {
                if (closeOnSelect) {
                    handleClose();
                }
